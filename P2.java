...
import programs.LitTuple;
	...
	static void initiallizeTables() throws Exception{
		...
		String s;
		BufferedReader br;
		br = new BufferedReader(new InputStreamReader(new FileInputStream("src/programs/symtable.txt")));
		while((s = br.readLine()) != null) {
			StringTokenizer st = new StringTokenizer(s, "\t", false);
			symtable.add(new SymTuple(st.nextToken(), st.nextToken(), ""));
		}
		br.close();
		br = new BufferedReader(new InputStreamReader(new FileInputStream("src/programs/littable.txt")));
		while((s = br.readLine()) != null) {
			StringTokenizer st = new StringTokenizer(s, "\t", false);
	...
	static void pass2() throws Exception{
		BufferedReader input = new BufferedReader(new InputStreamReader(new FileInputStream("src/programs/output_pass1.txt")));
		out_pass2 = new PrintWriter(new FileWriter("src/programs/output_pass2.txt"), true);
		String s;
		out_pass2.println("Name: Shardul Ajmera\nRoll no.: TACO22152");
//		input.readLine();
